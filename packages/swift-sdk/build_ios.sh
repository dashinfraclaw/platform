...
if $BUILD_IOS; then
  ...
  # Changing "dev" to "debug" bcs Rust writes "dev" profile to "debug" dir for some reason
  if [ "$PROFILE" = "dev" ]; then
    PROFILE="debug"
  fi
  IOS_LIB="$TARGET_DIR/$IOS_TARGET/$PROFILE/librs_unified_sdk_ffi.a"
  IOS_HEADERS="$TARGET_DIR/$IOS_TARGET/$PROFILE/include"
...
if $BUILD_SIM; then
  ...
  SIM_LIB="$TARGET_DIR/$SIM_TARGET/$PROFILE/librs_unified_sdk_ffi.a"
  SIM_HEADERS="$TARGET_DIR/$SIM_TARGET/$PROFILE/include"
...
if $BUILD_MAC; then
  ...
  MAC_LIB="$TARGET_DIR/$MAC_TARGET/$PROFILE/librs_unified_sdk_ffi.a"
  MAC_HEADERS="$TARGET_DIR/$MAC_TARGET/$PROFILE/include"
...
if $BUILD_INTEL_MAC; then
  ...
  INTEL_MAC_LIB="$TARGET_DIR/$INTEL_MAC_TARGET/$PROFILE/librs_unified_sdk_ffi.a"
  INTEL_MAC_HEADERS="$TARGET_DIR/$INTEL_MAC_TARGET/$PROFILE/include"
